...
url=leaderData[3];var li1=document.createElement('li');var min=Math.floor(leaders[i].ts/60);var sec=leaders[i].ts-(min*60);var ts=min+" min "+sec+" sec";$(li1).html('<a alt="'+id+'" href="'+url+'"><img target="_blank" width="128" height="90"  class="leaderImage" src="'+image+'"></a><div class="leaderName">'+name
+'</div><div class="leaderTime">'+ts+'</div>');$("#leaders").append(li1);}}}
function vote(streamNum){socket.send({vote:streamNum});}
function flag(streamNum){socket.send({flag:streamNum});}
function getMovie(){var movieName='Webcam';if(isFlashReady){try
{movie=document[movieName];movie=(movie==null||movie==undefined)?window[movieName]:movie;}
catch(e)
{return null;}
return movie;}
else
